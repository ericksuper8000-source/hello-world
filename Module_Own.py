...
Set_Pokecito = {'Vaporeon'}

Set_Conjunto_Poke.update(Set_Pokecito)
...
def Calculo(Lista):
    Elemento_Menor = min(Lista)
    Elemento_Mayor = max(Lista)
...
def Usuario(Nombre, Sexo):
    Genero = Sexo.lower()
    if (Genero == 'masculino'):
        print (f'{Nombre}, usted es un hombre')
    else:
        print (f'{Nombre}, usted es una mujer')
